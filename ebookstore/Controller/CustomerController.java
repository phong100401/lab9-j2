...
public class CustomerController {
    ...
    public int checkEmail(Users user, String email) {
        try (
                Connection conn = DriverManager.getConnection(url, username, password);
                Statement stmt = conn.createStatement();
        ) {
            int row = 0;
            String select = "Select * from customers where customerID=" + user.getId() + " AND email ='" + email + "'";
            ResultSet rset = stmt.executeQuery(select);
            while (rset.next()) {
                ++row;
            }
    ...
    public int checkPhone(String phone) {
        try (
            ...
            int row = 0;
            String select = "Select * from customers where phone ='" + phone + "'";
            ResultSet rset = stmt.executeQuery(select);
            while (rset.next()) {
                ++row;}
            if(row==0){
